...
export default function Dashboard() {
  ...
  return (
    <div className="px-10 text-white/80">
      <h1 className="text-5xl mb-10 text-primary text-center font-bold">
        Welcome to Tali&#39;s Pot
        <h4 className="text-xl text-center"> A Taste of Perfection!</h4>
      </h1>

    
      <div
        id="scroll-bar"
        className="scroll-bar w-full overflow-x-scroll flex flex-wrap gap-10 px-10 md:px-0">
        {foods.map((food) => {
          ...
          return food.id % 2 === 0 ? (
            ...
            <NavLink
              to={`/foods/${food.id}`}
              key={food.id}
              className="w-full flex justify-end rounded-lg md:rounded-none overflow-hidden">
              <div className="  w-full md:w-2/3 h-auto  flex-col md:flex-row flex">
                <div className="w-full">
                  <img
                    className="w-full h-[300px] md:rounded-l-full"
                    src={food.images[0]}
                    alt={food.name}
                  />
                </div>
