...
export default function Payment({food, currentImg, total, quantity, closeOrder}) {
  ...
  return (
    <div className=" w-full h-auto sm:w-4/5 md:w-1/2 bg-primary p-5 flex md:flex-row  flex-col relative rounded-4xl">
        ...
        <div className="w-full flex pt-10 flex-col space-y-5">
        ...
        <div onClick={closeOrder} className="absolute top-5 right-5 p-2" >
          <div  className={`w-7 transform transition-all p-0.5 duration-300 hover:bg-red-500  bg-white/80 ${toggle && "-rotate-45 translate-y-1"} `} ></div>
          <div className={`w-7 transform transition-all p-0.5  duration-300 hover:bg-red-500 bg-white/80 ${toggle && "rotate-45"}`}></div>
        </div>
    </div>
  )
}
